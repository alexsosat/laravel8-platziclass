...
@section('content')
    <div class="grid grid-cols-3 gap-4">
            ...
            <ul class="flex flex-col">
                ...
                @foreach ($course->posts as $post)
                    <li class="flex items-center text-gray-600 mt-2">
                        ...
                        @if ($post->free)
                            <span
                                class="text-xs text-gray-500 font-semibold bg-gray-300 rounded-full ml-auto px-2">Gratis</span>
                        @endif
                    </li>
                @endforeach
            </ul>
        ...
        <div class="text-gray-700 col-span-2">
            ...
            <h2 class="text-4xl my-2">{{ $course->name }}</h2>
            <p>{{ $course->description }}</p>
            <div class="flex mt-3">
                <img src="{{ $course->user->avatar }}" alt="imagen del usuario" class="h-10 w-10 rounded-full mr-2">
                <div>
                ...
                @foreach ($course->similar() as $course)
                    <div class="bg-white shadow-lg rounded-lg px-4 py-6 text-center">
                        <a href="{{ route('course', $course->slug) }}">
                            <img src="{{ $course->image }}" alt="class-image" class="rounded-md mb-2">
